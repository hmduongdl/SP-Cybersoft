...
const OPTION_IDLE =
  "border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-300";

function GeneratingAnimation() {
  const [textIndex, setTextIndex] = useState(0);
  const texts = [
    "AI đang phân tích thói quen của bạn...",
    "Đang phân bổ các công việc phù hợp...",
    "Thiết lập các khung giờ tối ưu...",
    "Vẽ thời khóa biểu...",
    "Hoàn thiện thời khóa biểu..."
  ];
  ...
  return (
    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white dark:bg-slate-950 p-8">
      <motion.div
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.5 }}
        className="w-16 h-16 rounded-2xl bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center mb-6 ring-4 ring-indigo-50 dark:ring-indigo-900/20"
      >
      ...
      <motion.p
        ...
        className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-10 text-center"
      >
        {texts[textIndex]}
        <motion.span
      ...
      {/* Table Drawing Animation */}
      <div className="w-full max-w-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div className="h-8 border-b border-slate-200 dark:border-slate-800 flex items-center px-3 gap-2 bg-slate-100/50 dark:bg-slate-800/50">
        ...
        <div className="p-5 space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <motion.div
              ...
              transition={{ duration: 0.8, delay: i * 0.5, ease: "circOut" }}
              className="flex gap-3 items-center"
            >
              <div className="h-5 w-1/4 bg-slate-200 dark:bg-slate-800 rounded-md shrink-0" />
              <div className="h-5 w-3/4 bg-indigo-100 dark:bg-indigo-900/40 border border-indigo-200 dark:border-indigo-800/50 rounded-md" />
            </motion.div>
          ))}
        </div>
      </div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      const [res] = await Promise.all([
        ...
        new Promise(resolve => setTimeout(resolve, 5500)) // Artificial delay for animation
      ]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <motion.div
        ...
        className="relative w-full max-w-lg bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden min-h-[500px] flex flex-col"
      >
          ...
          {loading ? (
            ...
            <motion.div key="content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col">
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-1.5">
                  {current.title}
                </h2>
                    ...
                    >
                      ...
                      {step === 1 && (
                        <div className="space-y-3">
                          {[
                            ...
                            >
                              <div className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-all ${config.is_job_flexible === value ? "border-indigo-500 bg-indigo-500" : "border-slate-300 dark:border-slate-700"}`}>
                                {config.is_job_flexible === value && <span className="w-1.5 h-1.5 rounded-full bg-white" />}
                              </div>
                              <div className="text-left">
                                <div className="font-semibold">{label}</div>
                                <div className="text-xs font-normal text-slate-500 dark:text-slate-400 mt-0.5">{desc}</div>
                              </div>
                            </button>
                          ))}
                        </div>
                          ...
                          {[
                            ...
                            >
                              ...
                              {config.best_energy_time === value && (
                                <CheckCircle2 className="ml-auto w-5 h-5 text-indigo-600 dark:text-indigo-500 shrink-0" />
                              )}
                            </button>
                          ))}
                        </div>
                          ...
                          <div className="grid grid-cols-4 gap-2 mt-2">
                            {[30, 45, 60, 90, 120].map((v) => (
                              <button
                                ...
                                className={`${OPTION_BASE} justify-center py-2 text-xs ${config.max_learning_time === v ? "border-rose-500 bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300" : OPTION_IDLE}`}
                              >
                                {formatMinutes(v)}
                              </button>
                            ))}
                      ...
                      {step === 5 && (
                        <div className="space-y-4">
                          ...
                          >
                            <div className={`relative w-12 h-6 rounded-full transition-colors shrink-0 ${config.sync_task_manager ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-700"}`}>
                              <motion.span
                ...
                <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 shrink-0">
                  <button
                    onClick={() => navigate(-1)}
                    disabled={step === 0}
                    className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all disabled:opacity-30 disabled:pointer-events-none"
                  >
                  ...
                  {isLast ? (
                    <button
                      ...
                      className="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold bg-indigo-600 hover:bg-indigo-700 text-white transition-all shadow-sm"
                    >
