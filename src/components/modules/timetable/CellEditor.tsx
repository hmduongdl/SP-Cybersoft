...
export default function CellEditor({
  ...
  if (!open) {
    if (isDeadline) {
      return (
        <div
          className="rounded bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 px-1.5 py-1 cursor-pointer hover:brightness-95 transition-all"
          ...
          {localItems.length > 0 && (
            <ul className="space-y-0.5">
              {localItems.map((item, i) => (
                <li key={i} className="flex items-start gap-1 text-[11px] text-red-700 dark:text-red-300 font-medium leading-tight">
    ...
    if (localItems.length === 1) {
      return (
        ...
        >
          <span className="text-[11px] text-slate-600 dark:text-slate-300 leading-snug pt-0.5">
            {localItems[0]}
          </span>
        </div>
      );
    ...
    return (
      <div
        className={`min-h-[28px] ${readOnly ? "" : "cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/40 rounded px-0.5 -mx-0.5 transition-colors"}`}
        onDoubleClick={() => !readOnly && setOpen(true)}
        title={readOnly ? undefined : "Double-click để sửa"}
      >
        <ul className="space-y-0.5">
          {localItems.map((item, i) => (
            <li key={i} className="flex items-start gap-1 text-[11px] text-slate-600 dark:text-slate-300 leading-tight">
