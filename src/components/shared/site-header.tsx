...
export function SiteHeader() {
  ...
  return (
    <header className="h-16 w-full bg-slate-900 border-b border-slate-800/80 sticky top-0 z-30 px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        ...
        <nav className="flex items-center space-x-1.5 text-sm font-medium" aria-label="Breadcrumb">
          <Link href="/dashboard" className="text-slate-400 hover:text-slate-200 transition">
            Trang chủ
          </Link>
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="flex items-center gap-2.5 p-1.5 rounded-full hover:bg-slate-800 transition text-slate-300 hover:text-white"
        >
          {userImage ? (
            <img
              src={userImage}
        ...
        {/* Dropdown Menu */}
        {dropdownOpen && (
          <div className="absolute right-0 mt-2 w-64 origin-top-right rounded-2xl border border-slate-800 bg-slate-900 p-2 shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
            {/* User Info Header */}
            <div className="px-4 py-3 border-b border-slate-850">
              ...
              <p className="text-sm font-semibold text-white mt-1 truncate">{userDisplayName}</p>
            ...
            <div className="mt-1">
              <button
                onClick={() => {
                  setDropdownOpen(false);
                  // Dynamic redirection/alert for demo
                  alert("Chức năng Thông tin tài khoản!");
                }}
                className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 rounded-lg transition"
              >
              ...
              <button
                ...
                className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-rose-400 hover:bg-rose-500/10 rounded-lg transition"
              >
